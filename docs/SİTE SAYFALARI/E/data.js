...
// ============================================================
//
// Not: Bu dosya `E/app.js` ile uyumlu kalması için her ders için hem:
// - `units`: Ünite -> alt konu (detaylı yapı)
// - `topics`: Düz liste (app.js'in beklediği)
// alanlarını barındırır. `topics` alanı aşağıda otomatik üretilir.
...
// `E/app.js` uyumu için `topics` alanlarını üret.
(function ensureFlatTopics(data) {
  const exams = Object.keys(data || {});
  exams.forEach(examId => {
    const ex = data[examId];
    if (!ex || !Array.isArray(ex.subjects)) return;
    ex.subjects.forEach(subj => {
      if (!subj) return;
      if (Array.isArray(subj.topics)) return;
      const units = Array.isArray(subj.units) ? subj.units : [];
      const flat = [];
      units.forEach(u => {
        const uTitle = (u && u.title) ? String(u.title) : '';
        const items = (u && Array.isArray(u.topics)) ? u.topics : [];
        items.forEach(t => {
          const topic = String(t);
          if (!uTitle || uTitle === 'Konular') flat.push(topic);
          else flat.push(`${uTitle} — ${topic}`);
        });
      });
      subj.topics = flat;
    });
